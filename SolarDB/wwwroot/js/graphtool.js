...
function formSubmit()
{
    ...
    let dataFetched = fetchData(start, end);
    if (dataValidated && dataFetched)
    {
    ...
    {
        if (!dataValidated) console.log("Data was not successfully validated.");
        if (!dataFetched) console.log("Something went wrong and new data could not be fetched.");
    }
}
 ...
 *              If not loaded, fetches missing chunk from api
 *          All dates are cached as contiguous blocks from the minimum to the maximum
*/
function fetchData(startDay, endDay)
{
    //Compare start and end day to the current data set

    //If data needs to be updated

    //Find suitable date range to keep contiguous data compared to cached data

    //Request data from Api
    let request = new XMLHttpRequest();
    let url = "https://localhost:44397/Solar/API";
    let queryString = "/?start=" + startDay + "&end=" + endDay;
    console.log("Attempting to connect to " + url + queryString);
    ...
    request.onload = function ()
    {
        ...
        {
            console.log("Something went wrong when connectiong to " + url);
        }
    }

    request.send(null);
    //Merge data with cached data
